 ...
 */
class Exercise2Test extends PetDomainForKata {
    @Test
    ...
    @Test
    void getAllPetsOfAllPeople() {
        Function<Person, Iterable<PetType>> function = person -> person.getPetTypes();
        MutableSet<PetType> petTypes = null;
        Assertions.assertThat(Sets.mutable.with(PetType.CAT, PetType.DOG, PetType.TURTLE, PetType.HAMSTER, PetType.BIRD, PetType.SNAKE)).isEqualTo(petTypes);
    }

    @Test
    void getFirstNamesOfAllPeople() {
        MutableList<String> firstNames = null;  // Transform this.people into a list of first names
        Assertions.assertThat(Lists.mutable.with("Mary", "Bob", "Ted", "Jake", "Barry", "Terry", "Harry", "John")).isEqualTo(firstNames);
    }
